...
!!! tip "什么是指令重排序"
    ...
    - 处理器优化重排：现代处理器采用了指令级并行技术来将多条指令重叠执行。
...
!!! tip "synchronized 底层原理了解吗？"
    ...
    synchronized关键字在Java中用于确保代码块在多线程环境下的线程安全。其底层原理主要基于监视器锁（Monitor Lock），实现了锁的获取和释放，保证了同一时刻只有一个线程可以执行同步代码块。在底层实现上，synchronized利用对象头中的标记字段（Mark Word）来存储锁信息，包括锁的状态和指向持有锁的线程的指针。

    锁有几种状态，包括无锁、偏向锁、轻量级锁和重量级锁，随着竞争情况的变化，锁可以在这些状态之间升级和降级。偏向锁优化单一线程访问，轻量级锁应用于短时间内的线程争用，而重量级锁涉及到系统级的互斥量，适用于长时间的线程竞争。

    当线程进入或退出一个synchronized同步块时，它会尝试获取或释放与对象关联的监视器锁。如果锁已被其他线程持有，则当前线程将等待，直到锁被释放。这个机制保证了对共享资源的安全访问，防止了数据的不一致性和竞态条件。
