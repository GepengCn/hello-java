...
!!! tip "内存分配和回收原则"
    ...
    如果对象在 Eden 出生并经过一次 Minor GC 后仍能存活，并能被 Survivor 容纳的话，将被移动到 Survivor 空间中，并将对象年龄设为 1。
...
!!! tip "垃圾收集器有哪些？"
    ...
    Serial 老年代版本，一个单线程收集器。它主要有两大用途：一种用途是在 JDK1.5 以及以前的版本中与 Parallel Scavenge 收集器搭配使用，另一种用途是作为 CMS 收集器的后备方案。

    - [x] Parallel Old 收集器

    Parallel Scavenge 收集器的老年代版本。使用多线程和“标记-整理”算法。关注吞吐量。
    ...
    - 并行与并发：利用CPU多核来缩短停顿时间。
    - 分代收集：不用其他收集器配合，自己就能实现分代回收
...
!!! tip "类加载器介绍"
    ...
    - BootstrapClassLoader(启动类加载器)：加载 JDK 核心类库

    - ExtensionClassLoader(扩展类加载器)：加载/lib/ext下的jar包和类。

    - AppClassLoader(应用程序类加载器)：加载classpath下的所有 jar 包和类。
