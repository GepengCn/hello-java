...
!!! tip "字符串常量池"
    ...
    JVM 为了提升性能和减少内存消耗针对字符串专门开辟的一块区域，主要目的是为了避免字符串的重复创建。
...
!!! tip "直接内存怎么在java中应用的"
    ...
    NIO，引入了一种基于通道与缓存区的 I/O 方式，它可以直接使用 Native 函数库直接分配堆外内存，然后通过一个存储在 Java 堆中的 DirectByteBuffer 对象来引用这块内存。在一些场景中显著提高性能，因为避免了在 Java 堆和 Native 堆之间来回复制数据。
...
!!! tip "Java 对象的创建过程或者类的生命周期"
    ...
    - final 关键字修饰的变量初始值设置多少就是多少，不会用初始值。public static final int value=111 ，那么准备阶段 value 的值就被赋值为 111。
...
!!! tip "对象可以被回收，就代表一定会被回收吗？"
    ...
    - 要真正宣告一个对象死亡，至少要经历两次标记过程；可达性分析法中不可达的对象被第一次标记
...
!!! tip "如何判断一个常量是废弃常量？"
    ...
    没有任何 String 对象引用该字符串常量的话，就说明常量 "abc" 就是废弃常量
